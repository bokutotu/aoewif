...
use crate::schedule::{CudaBinding, LoopId, VerifiedCudaSchedule};

use super::{
    CodegenError, CudaSource, SourceBuilder, compare_operator, flattened_index, input_name,
...
pub fn generate_cuda(schedule: &VerifiedCudaSchedule) -> Result<CudaSource, CodegenError> {
    ...
    source.line("#include <cuda_runtime.h>");
    ...
    let loop_value = |loop_id: LoopId| -> Result<String, CodegenError> {
        ...
        Ok(match loop_axis.binding() {
            Some(binding) => cuda_binding_expression(binding).to_owned(),
            None => format!("loop{}", loop_id.index()),
        })
    };
    ...
    for loop_axis in parallel_loops
        .iter()
        .filter(|loop_axis| loop_axis.binding().is_none())
    {
    ...
    Ok(CudaSource {
        ...
        kernel_name: function.name().to_owned(),
    })
}

fn emit_scalar_body(
    source: &mut SourceBuilder,
    schedule: &VerifiedCudaSchedule,
...
fn scalar_operation_expression(
    ...
    match operation {
        ...
        ScalarOperationKind::Add { lhs, rhs } => {
            Ok(format!("__fadd_rn({}, {})", value(*lhs)?, value(*rhs)?))
        }
        ScalarOperationKind::Sub { lhs, rhs } => {
            Ok(format!("__fsub_rn({}, {})", value(*lhs)?, value(*rhs)?))
        }
        ScalarOperationKind::Mul { lhs, rhs } => {
            Ok(format!("__fmul_rn({}, {})", value(*lhs)?, value(*rhs)?))
        }
        ScalarOperationKind::Div { lhs, rhs } => {
            Ok(format!("__fdiv_rn({}, {})", value(*lhs)?, value(*rhs)?))
        }
...
fn cuda_binding_expression(binding: CudaBinding) -> &'static str {
    match binding {
        CudaBinding::BlockX => "((size_t)blockIdx.x)",
        CudaBinding::BlockY => "((size_t)blockIdx.y)",
        CudaBinding::BlockZ => "((size_t)blockIdx.z)",
        CudaBinding::ThreadX => "((size_t)threadIdx.x)",
        CudaBinding::ThreadY => "((size_t)threadIdx.y)",
        CudaBinding::ThreadZ => "((size_t)threadIdx.z)",
    }
}
